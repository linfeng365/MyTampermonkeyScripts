...
def get_temp_preview_path(view):
    ...
    if file_name:
        tmp_filename = os.path.splitext(os.path.basename(file_name))[0] + '.html'
    else:
        # Unsaved views have no source filename, so keep the view ID fallback.
...
def get_temp_preview_dir(view):
    ...
    file_name = view.file_name()
    if file_name:
        # Keep previews beside their saved Markdown source files.
        return os.path.dirname(file_name)

    # Unsaved views have no source directory, so use the configured temporary
    # directory or fall back to the operating system's temporary directory.
    ...
    if path_tempfile:
        ...
        if os.path.isabs(path_tempfile):  # absolute path or not
            ...
            tmp_dir = os.path.join(tempfile.gettempdir(), path_tempfile)
    else:
...
class Compiler(object):
    ...
    def run(self, view, wholefile=False, preview=False):
        ...
        if self.settings.get('html_simple', False):
            ...
            html += self.get_title()
            html += '</head><body data-theme="{}">'.format(self.settings.get('theme', 'auto'))
